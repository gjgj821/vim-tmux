...
set fdm=marker
"关掉映射,经常带来麻烦
nnoremap <silent> <space> @=((foldclosed(line('.')) < 0) ? 'zc' : 'zo')<CR> "使用空格打开或关闭折叠
"}}}
...
" 的作者居然都没想到这点 。给出的推荐配置中还在纠结于 <TAB>,<ENTER>。这也是VISUAL ASSIST X推荐的方式。
...
inoremap <expr><Enter>  pumvisible() ? "\<C-Y>" : "\<Enter>"    "赋予 <Enter> <C-Y> 的功能。 <C-Y> 的功能是选择选项之后关闭弹出框。这么配置之后，同时也把回车键的换行效果取消了，免得选择之后光标却在下一行********************
"" 类似于AutoComplPop用法 .
let g:neocomplcache_enable_auto_select = 1
